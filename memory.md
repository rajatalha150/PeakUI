# ViewLlama | Local AI Studio - Project Memory
...
- **Managed Open Claw workspace** — Docker now mounts `${OPENCLAW_HOST_WORKSPACE_DIR:-/tmp/viewllama-openclaw-workspace}` into the app container at `/mnt/openclaw/workspace`, and the runtime creates a host-style alias so shell/code prompts can reliably use `/tmp/viewllama-openclaw-workspace`
