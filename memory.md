...
- Follow-up UWAF performance work made the keepalive use a no-network `level=cached` status path, removed duplicate stealth preflight from the action runner, capped search-provider attempts, shortened provider navigation timeouts, weighted provider scoring toward recent success/latency, and appended compact runtime/provider context so WorkSpaces knows the active direct/stealth providers and approved dark-web search engine IDs.
...
- Settings → Generation exposes optional local-only WorkSpaces model keep-alive; PeakUI does not send request-level `keep_alive` by default and never sends it to Ollama `*:cloud` aliases, while RAG embeddings still avoid keep-alive overrides
...
- **Native-first Ollama lifecycle** for WorkSpaces chat/generate requests — the app leaves request-level `keep_alive` off by default, keeps it as an opt-in local-only tuning control, never sends it to Ollama `*:cloud` aliases, and does not use background warmup prompts
