...
- Settings → Generation exposes configurable WorkSpaces model keep-alive; PeakUI sends Ollama `keep_alive` on chat/generate requests so local and Ollama-hosted cloud models stay warm between turns, while RAG embeddings still avoid keep-alive overrides
...
- **Terminal-like Ollama lifecycle** for WorkSpaces chat/generate requests — the app sends configurable `keep_alive` (default `30m`) without background warmup prompts, while embeddings keep native Ollama lifecycle behavior
