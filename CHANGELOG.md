...
- **Tests:** new `src/lib/chat-sessions-lean.test.ts` (4 tests) locks in the lean projection — `listChatSessions` omits `messages` and returns `messages: []`; `getChatSessionById` returns the full transcript; ownership check returns null for other users. `npx tsc --noEmit`, `npm test` (520 tests), and `npm run build` all pass.
