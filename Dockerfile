...
CMD ["sh", "-c", "mkdir -p /mnt/openclaw/workspace /var/lib/peakui && if [ -z \"${JWT_SECRET:-}\" ] || [ ${#JWT_SECRET} -lt 32 ]; then if [ -f /var/lib/peakui/jwt-secret ]; then export JWT_SECRET=\"$(cat /var/lib/peakui/jwt-secret)\"; else export JWT_SECRET=\"$(head -c 48 /dev/urandom | base64 | tr -d '\\n' | cut -c1-64)\"; printf '%s' \"$JWT_SECRET\" >/var/lib/peakui/jwt-secret; chmod 600 /var/lib/peakui/jwt-secret; fi; fi && if [ \"${OPENCLAW_HOST_WORKSPACE_DIR:-/tmp/peakui-openclaw-workspace}\" != \"/mnt/openclaw/workspace\" ] && [ ! -e \"${OPENCLAW_HOST_WORKSPACE_DIR:-/tmp/peakui-openclaw-workspace}\" ]; then mkdir -p \"$(dirname \"${OPENCLAW_HOST_WORKSPACE_DIR:-/tmp/peakui-openclaw-workspace}\")\" && ln -s /mnt/openclaw/workspace \"${OPENCLAW_HOST_WORKSPACE_DIR:-/tmp/peakui-openclaw-workspace}\"; fi && npx prisma db push && node server.js"]
