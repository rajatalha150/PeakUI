...
OPENCLAW_HOST_WORKSPACE_DIR=\"${OPENCLAW_HOST_WORKSPACE_DIR:-/tmp/peakui-openclaw-workspace}\" && \
if [ \"$OPENCLAW_HOST_WORKSPACE_DIR\" != \"/mnt/openclaw/workspace\" ] && [ ! -e \"$OPENCLAW_HOST_WORKSPACE_DIR\" ] && ! printf '%s' \"$OPENCLAW_HOST_WORKSPACE_DIR\" | grep -qE '^[A-Za-z]:'; then \
  mkdir -p \"$(dirname \"$OPENCLAW_HOST_WORKSPACE_DIR\")\" && ln -s /mnt/openclaw/workspace \"$OPENCLAW_HOST_WORKSPACE_DIR\"; \
fi && npx prisma db push && node server.js"]
