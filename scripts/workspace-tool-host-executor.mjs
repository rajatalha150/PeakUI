...
const bindHost = process.env.WORKSPACE_TOOL_HOST_EXECUTOR_BIND || '127.0.0.1'
const port = Number(process.env.WORKSPACE_TOOL_HOST_EXECUTOR_PORT || '4318')
const sharedToken = process.env.WORKSPACE_TOOL_HOST_EXECUTOR_TOKEN || ''
const shellPath = resolveShellPath()
const hostWorkspaceDir = (process.env.WORKSPACE_TOOL_HOST_WORKSPACE_DIR || '').trim()
const containerWorkspaceRoot = (process.env.WORKSPACE_TOOL_HOST_WORKSPACE_CONTAINER_ROOT || '/mnt/workspace-tool/workspace').trim()
...
function resolveShellPath() {
  const configured = (process.env.WORKSPACE_TOOL_HOST_EXECUTOR_SHELL || '').trim()
...
if (!sharedToken) {
  console.error('WORKSPACE_TOOL_HOST_EXECUTOR_TOKEN is required.')
  process.exit(1)
}
...
/**
 * The app container may send cwd as a container path (e.g. /mnt/workspace-tool/workspace/...).
...
server.listen(port, bindHost, () => {
  console.log(`Workspace Tool host executor listening on http://${bindHost}:${port} (shell=${shellPath}, platform=${process.platform})`)
})
