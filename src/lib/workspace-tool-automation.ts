...
import {
  getMountedWorkspaceToolHostRoots,
  getMountedWorkspaceToolRootDetails,
...
import { queueAutomationExecution, processQueuedAutomationRuns } from './workspace-tool-automation-execution'
...
async function getAutomationToolAccess(userId: string) {
  ...
  return {
    settings,
    effectiveToolAccess: buildEffectiveWorkspaceToolAccess(settings, permissions),
  }
}
...
async function authorizeFileMonitorTarget(userId: string, rawTarget: string) {
  ...
  const approvedRoots = parseAllowedWorkspaceToolPaths(settings.workspaceToolAllowedPaths)
  if (approvedRoots.length === 0) {
    throw new Error('File monitors require at least one approved WorkSpaces filesystem root.')
  }

  const mountedRoots = getMountedWorkspaceToolHostRoots()
...
function translateMonitorTargetToContainerPath(hostTarget: string): string {
  ...
  const roots = getMountedWorkspaceToolRootDetails()
  const matched = roots
