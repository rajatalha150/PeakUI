 ...
 * every round (`OpenClawWorkspace.tsx` builds `conversationMessages` from it
 ...
 * `OpenClawWorkspace.tsx`. A hidden `role: 'user'` message whose content
...
const TOOL_RESULT_HEADER_RE =
  /^(?:Shell command|Filesystem tool|Code execution|Web research tool|Browser tool|Tax return PDF tool|PDF document tool|Excel workbook tool|Word document tool|CSV export tool|Email writer tool|Markdown document tool|Slide deck tool|Archive tool|Calendar tool|Mermaid diagram tool|URL fetch and summarize tool) result:/
