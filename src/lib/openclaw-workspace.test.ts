...
describe('recordDarkWebSearchNote — filesystem behavior', () => {
  it('appends to an existing file instead of overwriting it', async () => {
    ...
    // production data. The writer uses getOpenClawWorkspaceHostRoot()
    // and getUwafDarkWebDir() as-is; we override the env var.
    const previous = process.env.OPENCLAW_HOST_WORKSPACE_DIR
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'peakui-darkweb-'))
    process.env.OPENCLAW_HOST_WORKSPACE_DIR = tmpRoot
    try {
      ...
      if (previous === undefined) {
        delete process.env.OPENCLAW_HOST_WORKSPACE_DIR
      } else {
        process.env.OPENCLAW_HOST_WORKSPACE_DIR = previous
      }
  ...
  it('returns ok:false (does not throw) when the workspace directory cannot be written', async () => {
    ...
    process.env.OPENCLAW_HOST_WORKSPACE_DIR = blockedDir
    try {
      const result = await recordDarkWebSearchNote(makeEntry())
  ...
  it('uses the host workspace root when OPENCLAW_HOST_WORKSPACE_DIR is set', () => {
    const previous = process.env.OPENCLAW_HOST_WORKSPACE_DIR
    process.env.OPENCLAW_HOST_WORKSPACE_DIR = '/tmp/test-workspace-xyz'
    try {
      expect(getOpenClawWorkspaceHostRoot()).toBe('/tmp/test-workspace-xyz')
    } finally {
      if (previous === undefined) {
        delete process.env.OPENCLAW_HOST_WORKSPACE_DIR
      } else {
