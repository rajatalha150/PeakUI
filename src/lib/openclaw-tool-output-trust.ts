 ...
 * `<openclaw_tool>` tag, or a `<system>` marker — and a local model reading
...
export function neutralizeUntrustedDelimiters(content: string): string {
  return content
    ...
    .replace(/<\/?openclaw_tool\b[^>]*>/gi, (match) =>
      match.replace(/</g, '[').replace(/>/g, ']').replace(/openclaw_tool/gi, 'openclaw_tool_'),
    )
