...
const WORKSPACE_METADATA_FILENAME = '.workspace-tool-workspace.json'
...
interface WorkspaceToolWorkspaceMetadata {
  id: string
  slug: string
  name: string
  description: string
...
export interface WorkspaceToolWorkspaceContext {
  workspace: WorkspaceToolWorkspaceRecord
...
export interface CreateWorkspaceToolWorkspaceInput {
  name: string
  description?: string
  autoGitBackup?: boolean
}

export interface UpdateWorkspaceToolWorkspaceInput {
  name?: string
  description?: string
  autoGitBackup?: boolean
}
...
function buildWorkspaceContainerPath(userId: string, slug: string): string {
  return path.join(getWorkspaceToolWorkspaceContainerRoot(), 'users', userId, 'workspaces', slug)
}

function buildWorkspaceHostPath(relativePath: string): string {
  return path.join(getWorkspaceToolWorkspaceHostRoot(), relativePath)
}
...
async function readWorkspaceMetadata(containerPath: string): Promise<WorkspaceToolWorkspaceMetadata | null> {
  try {
    const raw = await fs.readFile(workspaceMetadataPath(containerPath), 'utf8')
    const parsed = JSON.parse(raw) as Partial<WorkspaceToolWorkspaceMetadata>
    if (
...
async function writeWorkspaceMetadata(containerPath: string, metadata: WorkspaceToolWorkspaceMetadata): Promise<void> {
  await fs.writeFile(
...
async function buildWorkspaceRecord(
  userId: string,
  containerPath: string,
  metadata: WorkspaceToolWorkspaceMetadata,
): Promise<WorkspaceToolWorkspaceRecord> {
...
async function runGitInWorkspace(containerPath: string, args: string[]): Promise<string> {
  const { stdout } = await execFileAsync('git', args, {
    ...
    env: {
      ...
      GIT_AUTHOR_NAME: 'WorkspaceTool Workspace',
      GIT_AUTHOR_EMAIL: 'workspace-tool@local.invalid',
      GIT_COMMITTER_NAME: 'WorkspaceTool Workspace',
      GIT_COMMITTER_EMAIL: 'workspace-tool@local.invalid',
    },
  })
  return stdout.trim()
}
...
async function maybeSnapshotWorkspaceGitBackup(
  ...
  metadata: WorkspaceToolWorkspaceMetadata,
  reason: string,
): Promise<WorkspaceToolWorkspaceMetadata> {
  ...
  try {
    ...
    const nextMetadata: WorkspaceToolWorkspaceMetadata = {
      ...metadata,
...
async function ensureWorkspaceContainerRoot(userId: string): Promise<string> {
  ...
  const root = path.join(getWorkspaceToolWorkspaceContainerRoot(), 'users', userId, 'workspaces')
  await fs.mkdir(root, { recursive: true })
  return root
}

async function loadWorkspaceByDirectoryName(userId: string, directoryName: string): Promise<WorkspaceToolWorkspaceRecord | null> {
...
export async function listWorkspaceToolWorkspaces(userId: string): Promise<WorkspaceToolWorkspaceRecord[]> {
  const root = await ensureWorkspaceContainerRoot(userId)
  const entries = await fs.readdir(root, { withFileTypes: true })
  ...
  return workspaces
    .filter((workspace): workspace is WorkspaceToolWorkspaceRecord => Boolean(workspace))
    .sort((a, b) => a.name.localeCompare(b.name))
}

export async function ensureDefaultWorkspaceToolWorkspace(userId: string): Promise<WorkspaceToolWorkspaceRecord> {
  const existing = await listWorkspaceToolWorkspaces(userId)
  if (existing.length > 0) {
    return existing[0]
  }

  return createWorkspaceToolWorkspace(userId, {
...
export async function createWorkspaceToolWorkspace(
  userId: string,
  input: CreateWorkspaceToolWorkspaceInput,
  forcedSlug?: string,
): Promise<WorkspaceToolWorkspaceRecord> {
  ...
  const metadata: WorkspaceToolWorkspaceMetadata = {
...
export async function getWorkspaceToolWorkspaceById(userId: string, workspaceId: string): Promise<WorkspaceToolWorkspaceRecord | null> {
...
export async function updateWorkspaceToolWorkspace(
  userId: string,
  workspaceId: string,
  input: UpdateWorkspaceToolWorkspaceInput,
): Promise<WorkspaceToolWorkspaceRecord> {
  const found = await loadWorkspaceByIdInternal(userId, workspaceId)
  if (!found) {
...
export async function getWorkspaceToolWorkspaceContext(
  userId: string,
  workspaceId?: string,
): Promise<WorkspaceToolWorkspaceContext> {
  const workspace = workspaceId
    ? await getWorkspaceToolWorkspaceById(userId, workspaceId)
    : await ensureDefaultWorkspaceToolWorkspace(userId)

  const resolvedWorkspace = workspace || await ensureDefaultWorkspaceToolWorkspace(userId)
