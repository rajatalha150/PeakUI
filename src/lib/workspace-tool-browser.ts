...
import type { WorkspaceToolBrowserMode } from './settings'
...
export interface WorkspaceToolBrowserRequest {
...
export interface WorkspaceToolBrowserSettings {
  workspaceToolBrowserMode: WorkspaceToolBrowserMode
}

export interface WorkspaceToolBrowserLink {
  index: number
  text: string
  url: string
}

export interface WorkspaceToolBrowserFormField {
  name: string
  type: string
  value?: string
}

export interface WorkspaceToolBrowserForm {
  index: number
  action: string
  method: 'GET' | 'POST'
  fields: WorkspaceToolBrowserFormField[]
}

export interface WorkspaceToolBrowserResult {
  action: WorkspaceToolBrowserRequest['action']
  currentUrl: string
  title: string
  text: string
  html?: string
  links: WorkspaceToolBrowserLink[]
  forms: WorkspaceToolBrowserForm[]
...
export interface WorkspaceToolBrowserSubmitPreparation {
  formIndex: number
  submitUrl: string
  method: 'GET' | 'POST'
  values: Record<string, string>
...
function extractLinks(html: string, pageUrl: string): WorkspaceToolBrowserLink[] {
  const links: WorkspaceToolBrowserLink[] = []
...
function extractFormFields(formBody: string): WorkspaceToolBrowserFormField[] {
  const fields: WorkspaceToolBrowserFormField[] = []
...
function extractForms(html: string, pageUrl: string): WorkspaceToolBrowserForm[] {
  const forms: WorkspaceToolBrowserForm[] = []
...
function findForm(page: BrowserPageSnapshot, formIndex?: number): WorkspaceToolBrowserForm {
...
function findLink(page: BrowserPageSnapshot, input: { linkIndex?: number; linkText?: string }): WorkspaceToolBrowserLink {
...
function buildBrowserResult(
  ...
  options?: {
    mode?: WorkspaceToolBrowserRequest['mode']
    pendingFormValues?: Record<string, string>
    submitted?: WorkspaceToolBrowserResult['submitted']
  }
): WorkspaceToolBrowserResult {
...
export function prepareWorkspaceToolBrowserSubmit(
  request: WorkspaceToolBrowserRequest,
  session: BrowserSessionState
): WorkspaceToolBrowserSubmitPreparation {
...
export function buildWorkspaceToolBrowserApprovalPayload(
  request: WorkspaceToolBrowserRequest,
  preparation: WorkspaceToolBrowserSubmitPreparation
) {
  return {
...
export async function runWorkspaceToolBrowserAction(
  userId: string,
  request: WorkspaceToolBrowserRequest,
  settings: WorkspaceToolBrowserSettings
): Promise<WorkspaceToolBrowserResult> {
  if (settings.workspaceToolBrowserMode === 'deny') {
  ...
  if (settings.workspaceToolBrowserMode === 'read-only') {
  ...
  const submission = prepareWorkspaceToolBrowserSubmit(request, session)
...
export function getWorkspaceToolBrowserSession(userId: string, sessionId: string): BrowserSessionState {
