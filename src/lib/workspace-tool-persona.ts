export type WorkspaceToolPersonaTemplateId =
  | 'developer'
  | 'researcher'
  | 'writer'
  | 'analyst'
...
export interface WorkspaceToolPersona {
  name: string
  tone: string
  expertise: string
  boundaries: string
...
export interface WorkspaceToolUserProfile {
  name: string
  role: string
  preferences: string
  context: string
...
export const DEFAULT_WORKSPACE_TOOL_PERSONA: WorkspaceToolPersona = {
  name: 'WorkSpaces',
...
export const DEFAULT_WORKSPACE_TOOL_USER_PROFILE: WorkspaceToolUserProfile = {
  name: '',
  role: '',
  preferences: '',
  context: '',
...
export const WORKSPACE_TOOL_PERSONA_TEMPLATES: Record<
  Exclude<WorkspaceToolPersonaTemplateId, 'custom'>,
  WorkspaceToolPersona
> = {
  developer: {
    name: 'Code Partner',
...
export function buildPersonaBrief(persona: WorkspaceToolPersona): string {
...
export function buildUserProfileBrief(profile: WorkspaceToolUserProfile): string {
...
export function applyPersonaTemplate(
  templateId: WorkspaceToolPersonaTemplateId,
  currentPersona?: Partial<WorkspaceToolPersona>
): WorkspaceToolPersona {
  ...
  const template = WORKSPACE_TOOL_PERSONA_TEMPLATES[templateId]
  return {
    ...template,
    ...currentPersona,
...
export function normalizePersonaTemplateId(value: unknown): WorkspaceToolPersonaTemplateId {
  const valid: WorkspaceToolPersonaTemplateId[] = [
    'developer',
    'researcher',
    'writer',
    'analyst',
  ...
  return valid.includes(value as WorkspaceToolPersonaTemplateId) ? (value as WorkspaceToolPersonaTemplateId) : 'custom'
}
