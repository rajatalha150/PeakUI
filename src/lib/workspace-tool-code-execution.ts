...
import type { WorkspaceToolCodeExecutionMode } from './settings'
import { getWorkspaceToolWorkspaceContainerRoot, getWorkspaceToolWorkspaceHostRoot, isWindowsHostPath } from './workspace-tool-workspace'

export type WorkspaceToolCodeRuntime = 'python' | 'node'

export interface WorkspaceToolCodeExecutionRequest {
  runtime: WorkspaceToolCodeRuntime
  code: string
  filename?: string
...
export interface WorkspaceToolCodeExecutionResult {
  success: boolean
  runtime: WorkspaceToolCodeRuntime
  command: string
...
function resolveHostPathToContainer(hostPath: string): string {
  ...
  if (isWindowsHostPath(hostPath)) {
    // Windows host paths are bind-mounted at /mnt/workspace-tool/workspace on Windows compose,
    // and at /mnt/workspace-tool/projects for Desktop etc. Without exact mount metadata we
...
function sanitizeFilename(runtime: WorkspaceToolCodeRuntime, filename?: string): string {
...
export function prepareWorkspaceToolCodeExecutionRequest(
  request: WorkspaceToolCodeExecutionRequest,
  settings: WorkspaceToolCodeExecutionSettings
): WorkspaceToolCodeExecutionPreparation {
  if (settings.workspaceToolCodeExecutionMode === 'deny' && settings.workspaceToolHostAccessMode !== 'auto-approve') {
    throw new Error('Code execution sandbox is disabled')
  }

  const hostAccessEnabled = settings.workspaceToolHostAccessMode === 'auto-approve' || settings.workspaceToolHostAccessMode === 'ask-first' 
  ...
  if (hostAccessEnabled && (request.workspacePath?.startsWith('/') || isWindowsHostPath(request.workspacePath || ''))) {
    ...
    relativeWorkspacePath = isWindowsHostPath(request.workspacePath || '')
      ? path.relative(getWorkspaceToolWorkspaceHostRoot(), requestedHostPath) || '.'
    ...
    hostWorkspacePath = path.join(getWorkspaceToolWorkspaceHostRoot(), relativeWorkspacePath)
    containerWorkspacePath = path.join(getWorkspaceToolWorkspaceContainerRoot(), relativeWorkspacePath)
  }
...
export function buildWorkspaceToolCodeApprovalPayload(request: WorkspaceToolCodeExecutionPreparation) {
  return {
    runtime: request.runtime,
    code: request.code,
...
export async function runWorkspaceToolCodeExecution(
  request: WorkspaceToolCodeExecutionRequest,
  settings: WorkspaceToolCodeExecutionSettings,
  abortSignal?: AbortSignal
): Promise<WorkspaceToolCodeExecutionResult> {
  const prepared = prepareWorkspaceToolCodeExecutionRequest(request, settings)
  ...
  if (prepared.runtime === 'python') {
    const guardName = '__workspace-tool_guard__.py'
    ...
    const guardName = '__workspace-tool_guard__.cjs'
    const guardContainerPath = path.join(prepared.containerWorkspacePath, guardName)
