...
export async function createArchiveCanvasArtifact(input: {
  ...
  const content = input.archiveBytes.toString('base64')
  ...
  if (input.source && !sourceArtifactId) {
    const sourceMetadata = computeCanvasArtifactMetadata({
      ...
      bundleId,
      bundleName: input.bundleName ?? 'Generated Archive',
      bundleRole: input.source.bundleRole ?? 'archive-source',
    })
    ...
    const source = await input.tx.canvasArtifact.create({
      data: {
        ...
        previewSummary: sourceMetadata.previewSummary || 'Archive source document',
  ...
  const metadata = computeCanvasArtifactMetadata({
    ...
    bundleId,
    bundleName: input.bundleName ?? 'Generated Archive',
    bundleRole: input.bundleRole ?? 'generated-archive',
  })
  ...
  const created = await input.tx.canvasArtifact.create({
    data: {
      ...
      previewSummary: metadata.previewSummary || 'Generated archive',
