export type WorkspaceToolAgentMode = 'plan' | 'research' | 'execute' | 'review'
export type WorkspaceToolResponseStyle = 'concise' | 'structured' | 'deep'

export interface WorkspaceToolAgentPreferences {
  mode: WorkspaceToolAgentMode
  responseStyle: WorkspaceToolResponseStyle
...
export interface WorkspaceToolChecklistItem {
  id: string
  text: string
  completed: boolean
}

export interface WorkspaceToolTaskState {
  objective: string
  currentStatus: string
  nextStep: string
  doneCriteria: string
  checklist: WorkspaceToolChecklistItem[]
}

export const DEFAULT_WORKSPACE_TOOL_AGENT_PREFERENCES: WorkspaceToolAgentPreferences = {
  mode: 'plan',
...
export const DEFAULT_WORKSPACE_TOOL_TASK_STATE: WorkspaceToolTaskState = {
  objective: '',
  currentStatus: '',
  nextStep: '',
  doneCriteria: '',
...
export const WORKSPACE_TOOL_AGENT_MODE_OPTIONS: Array<{
  id: WorkspaceToolAgentMode
  label: string
  description: string
}> = [
  {
...
export const WORKSPACE_TOOL_RESPONSE_STYLE_OPTIONS: Array<{
  id: WorkspaceToolResponseStyle
  label: string
  description: string
}> = [
  {
...
export const WORKSPACE_TOOL_QUICK_PROMPTS: Array<{
  title: string
  description: string
  mode: WorkspaceToolAgentMode
  prompt: string
...
function getModeInstruction(mode: WorkspaceToolAgentMode): string {
  switch (mode) {
    case 'research':
...
function getResponseStyleInstruction(style: WorkspaceToolResponseStyle): string {
  switch (style) {
    case 'concise':
...
export function extractWorkspaceToolChecklistSuggestions(content: string): string[] {
...
export function createWorkspaceToolChecklistItems(lines: string[]): WorkspaceToolChecklistItem[] {
...
export function buildWorkspaceToolWorkspaceBrief(preferences: WorkspaceToolAgentPreferences): string {
  const lines = [
...
export function buildWorkspaceToolTaskStateBrief(taskState: WorkspaceToolTaskState): string {
  const lines: string[] = []
