...
export async function createCsvCanvasArtifact(input: {
  ...
  if (input.source && !sourceArtifactId) {
    const sourceMetadata = computeCanvasArtifactMetadata({
      ...
      bundleId,
      bundleName: input.bundleName ?? 'Generated CSV',
      bundleRole: input.source.bundleRole ?? 'csv-source',
    })
    ...
    const source = await input.tx.canvasArtifact.create({
      data: {
        ...
        previewSummary: sourceMetadata.previewSummary || 'CSV source document',
  ...
  const metadata = computeCanvasArtifactMetadata({
    ...
    bundleId,
    bundleName: input.bundleName ?? 'Generated CSV',
    bundleRole: input.bundleRole ?? 'generated-csv',
  })
  ...
  const created = await input.tx.canvasArtifact.create({
    data: {
      ...
      kind: 'data',
      extension: 'csv',
      size: input.csvBytes.byteLength,
      sessionId: input.sessionId,
      messageId: input.messageId ?? null,
      userId: input.userId,
      previewKind: metadata.previewKind,
      previewSummary: metadata.previewSummary || 'Generated CSV data',
