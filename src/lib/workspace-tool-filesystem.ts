...
import type {
  WorkspaceToolFileAccessMode,
  WorkspaceToolFileWriteMode,
} from './settings'
import {
...
export type WorkspaceToolFilesystemReadAction = 'list' | 'read' | 'stat'
export type WorkspaceToolFilesystemWriteAction = 'write' | 'append' | 'mkdir'
export type WorkspaceToolFilesystemAction = WorkspaceToolFilesystemReadAction | WorkspaceToolFilesystemWriteAction

export interface WorkspaceToolFilesystemRequest {
  action: WorkspaceToolFilesystemAction
  path: string
  content?: string
  createDirectories?: boolean
}

export interface WorkspaceToolFilesystemResult {
  action: WorkspaceToolFilesystemAction
  path: string
  kind: 'file' | 'directory'
  content?: string
...
export interface WorkspaceToolFilesystemAccessSettings {
...
export interface WorkspaceToolFilesystemWriteValidation {
  action: WorkspaceToolFilesystemWriteAction
  path: string
  contentBytes?: number
  createDirectories: boolean
}

export interface WorkspaceToolFilesystemApprovalPayload {
  action: WorkspaceToolFilesystemWriteAction
  path: string
  content?: string
  createDirectories?: boolean
...
export interface WorkspaceToolMountedRoot {
  label: string
  hostPath: string
...
export type WorkspaceToolFilesystemDiagnosticCode =
  | 'ok'
  | 'invalid_path'
...
export interface WorkspaceToolFilesystemDiagnostic {
  allowed: boolean
  code: WorkspaceToolFilesystemDiagnosticCode
  message: string
  actionRequired?: string
  requestedPath?: string
  normalizedPath?: string
  approvedRoot?: string
  mountedRoot?: WorkspaceToolMountedRoot
}

export interface WorkspaceToolFilesystemAccessStatus {
  readMode: WorkspaceToolFileAccessMode
  writeMode: WorkspaceToolFileWriteMode
  approvedReadRoots: string[]
  approvedWritableRoots: string[]
  mountedRoots: WorkspaceToolMountedRoot[]
  mountedWritableRoots: WorkspaceToolMountedRoot[]
  readReady: boolean
  writeReady: boolean
  warnings: string[]
}
...
function getMountedRoots(): WorkspaceToolMountedRoot[] {
  const roots = [
    {
      label: 'Host home tree',
      hostPath: normalizeAbsolutePath(process.env.WORKSPACE_TOOL_HOST_HOME_DIR || '/home'),
      containerPath: '/mnt/workspace-tool/home',
      writable: false,
    },
    {
      label: 'Host temp',
      hostPath: normalizeAbsolutePath(process.env.WORKSPACE_TOOL_HOST_TMP_DIR || '/tmp'),
      containerPath: '/mnt/workspace-tool/tmp',
      writable: false,
    },
    {
      label: 'Managed workspace',
      hostPath: getWorkspaceToolWorkspaceHostRoot(),
      containerPath: getWorkspaceToolWorkspaceContainerRoot(),
      writable: true,
    },
  ]
...
export function isWorkspaceToolFilesystemWriteAction(action: WorkspaceToolFilesystemAction): action is WorkspaceToolFilesystemWriteAction {
  return action === 'write' || action === 'append' || action === 'mkdir'
}

export function isWorkspaceToolFilesystemReadAction(action: WorkspaceToolFilesystemAction): action is WorkspaceToolFilesystemReadAction {
  return action === 'list' || action === 'read' || action === 'stat'
}

export function parseAllowedWorkspaceToolPaths(rawValue: string): string[] {
  return rawValue
    .split(/\r?\n/)
...
export function buildWorkspaceToolFilesystemApprovalPayload(
  request: WorkspaceToolFilesystemRequest,
  normalizedPath: string
): WorkspaceToolFilesystemApprovalPayload {
  return {
    action: request.action as WorkspaceToolFilesystemWriteAction,
    path: normalizedPath,
...
export function getMountedWorkspaceToolHostRoots(): string[] {
  return getMountedRoots().map(root => root.hostPath)
}

export function getMountedWorkspaceToolWritableRoots(): string[] {
  return getMountedRoots().filter(root => root.writable).map(root => root.hostPath)
}

export { isKnownWorkspaceToolHostPath, isWindowsHostPath } from './workspace-tool-path-check'
import { isKnownWorkspaceToolHostPath } from './workspace-tool-path-check'

export function isKnownWorkspaceToolHostPathWithMountedRoots(value: string): boolean {
  if (!value || typeof value !== 'string') return false
  const normalized = normalizeAbsolutePath(value)
  const prefixes = getMountedRoots().map(root => normalizeAbsolutePath(root.hostPath))
  return isKnownWorkspaceToolHostPath(value, prefixes)
}

export function getMountedWorkspaceToolRootDetails(): WorkspaceToolMountedRoot[] {
  return getMountedRoots().map(root => ({ ...root }))
}

function resolveMountedRootForPath(hostPath: string, options?: { writableOnly?: boolean }): WorkspaceToolMountedRoot | null {
...
export function buildWorkspaceToolFilesystemAccessStatus(
  settings: WorkspaceToolFilesystemAccessSettings
): WorkspaceToolFilesystemAccessStatus {
  const approvedReadRoots = parseAllowedWorkspaceToolPaths(settings.workspaceToolAllowedPaths)
  const approvedWritableRoots = parseAllowedWorkspaceToolPaths(settings.workspaceToolWritablePaths)
  const mountedRoots = getMountedWorkspaceToolRootDetails()
  const mountedWritableRoots = mountedRoots.filter(root => root.writable)
  const warnings: string[] = []

  if (settings.workspaceToolFileAccessMode === 'read-only' && approvedReadRoots.length === 0) {
    warnings.push('Read-only filesystem access is enabled, but no approved host read roots are configured.')
  }

  if (settings.workspaceToolFileWriteMode !== 'deny' && approvedWritableRoots.length === 0) {
  ...
  return {
    readMode: settings.workspaceToolFileAccessMode,
    writeMode: settings.workspaceToolFileWriteMode,
    approvedReadRoots,
    approvedWritableRoots,
    mountedRoots,
    mountedWritableRoots,
    readReady: settings.workspaceToolFileAccessMode === 'read-only'
      && approvedReadRoots.length > 0
      && approvedReadRoots.some(root => Boolean(resolveMountedRootForPath(root))),
    writeReady: settings.workspaceToolFileWriteMode !== 'deny'
...
export function diagnoseWorkspaceToolFilesystemRequest(
  request: WorkspaceToolFilesystemRequest,
  settings: WorkspaceToolFilesystemAccessSettings,
  options: { approvalTokenPresent?: boolean; hostAccessEnabled?: boolean } = {}
): WorkspaceToolFilesystemDiagnostic {
  ...
  if (looksRelative) {
    const workspaceRoot = getWorkspaceToolWorkspaceHostRoot()
    return {
      allowed: false,
      code: 'invalid_path',
  ...
  const isWriteAction = isWorkspaceToolFilesystemWriteAction(request.action)

  if (!isWriteAction && settings.workspaceToolFileAccessMode !== 'read-only') {
    return {
      allowed: false,
  ...
  if (isWriteAction && settings.workspaceToolFileWriteMode === 'deny' && !hostAccessEnabled) {
    return {
      allowed: false,
  ...
  const approvedRoots = isWriteAction
    ? parseAllowedWorkspaceToolPaths(settings.workspaceToolWritablePaths)
    : parseAllowedWorkspaceToolPaths(settings.workspaceToolAllowedPaths)
  ...
  if (!mountedRoot) {
    const mountedRoots = isWriteAction ? getMountedWorkspaceToolWritableRoots() : getMountedWorkspaceToolHostRoots()
    return {
      allowed: false,
  ...
  if (isWriteAction && settings.workspaceToolFileWriteMode === 'ask-first' && !options.approvalTokenPresent) {
    return {
      allowed: false,
...
async function resolveReadableContainerPaths(
  ...
  if (settings.workspaceToolFileAccessMode !== 'read-only') {
    throw new Error('Filesystem access is disabled')
  }

  const normalizedHostPath = normalizeAbsolutePath(requestedHostPath)
  const allowedRoots = parseAllowedWorkspaceToolPaths(settings.workspaceToolAllowedPaths)
  ...
  if (!mountedRoot) {
    throw new Error(`Requested path is outside the mounted host roots. Available mounted roots: ${getMountedWorkspaceToolHostRoots().join(', ')}`)
  }
...
async function resolveWritableContainerPaths(
  ...
  if (settings.workspaceToolFileWriteMode === 'deny') {
    throw new Error('Filesystem writes are disabled')
  }

  const normalizedHostPath = normalizeAbsolutePath(requestedHostPath)
  const allowedRoots = parseAllowedWorkspaceToolPaths(settings.workspaceToolWritablePaths)
  const authorizedRoot = findAuthorizedRoot(normalizedHostPath, allowedRoots)
  ...
  if (!mountedRoot) {
    throw new Error(`Requested path is outside the mounted writable roots. Available writable roots: ${getMountedWorkspaceToolWritableRoots().join(', ')}`)
  }
...
export async function validateWorkspaceToolFilesystemWriteRequest(
  request: WorkspaceToolFilesystemRequest,
  settings: WorkspaceToolFilesystemAccessSettings
): Promise<WorkspaceToolFilesystemWriteValidation> {
  if (!isWorkspaceToolFilesystemWriteAction(request.action)) {
...
export async function runWorkspaceToolFilesystemRequest(
  request: WorkspaceToolFilesystemRequest,
  settings: WorkspaceToolFilesystemAccessSettings
): Promise<WorkspaceToolFilesystemResult> {
  if (isWorkspaceToolFilesystemReadAction(request.action)) {
  ...
  const validation = await validateWorkspaceToolFilesystemWriteRequest(request, settings)
