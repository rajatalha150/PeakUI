...
export function buildOpenClawSystemPrompt(context: OpenClawPromptContext): string {
  ...
  const lines: string[] = [
    ...
    'LEAD WITH THE TOOL BLOCK: When you intend to call a tool, the very first thing in your response must be the complete `<openclaw_tool>` wrapper. Do NOT start with prose such as "Running shell command:", "Now writing...", "Next step:...", or "I will...". Those phrases are stripped by the runtime and the tool block is treated as missing, causing a wasted turn. Put any explanation AFTER the wrapper or omit it entirely.',
