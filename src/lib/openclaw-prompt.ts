...
export function buildChatInternetToolPrompt(): string {
  return [
    ...
    '',
    'WHEN TO SEARCH: Search for any question about current events, prices, people, products, software versions, statistics, news, health guidance, legal matters, or anything that could have changed since your training cutoff. If you are not certain a fact is still current, search rather than answering from memory. Do not search for stable knowledge (math, history, coding patterns) unless the user explicitly asks for verification.',
    '',
    'QUERY RULES:',
    '- Write search queries as concise search phrases (3-8 words), not natural-language questions.',
    '- Be specific: include years, model numbers, product names, locations, or domain terms.',
    '- Bad: "best phone". Good: "best budget phone under $500 2025".',
    '- Bad: "python sort". Good: "python 3.12 sort list of dicts by key".',
    '- Do not use search operators like site:, -, or quotes unless the user explicitly asks.',
    '- If the user asks in a non-English language, also search with an English translation of the key terms.',
    '',
    'USING RESULTS:',
    '- Read the provided source snippets carefully before answering.',
    '- Cite every factual claim derived from search results with inline markers like [^1] using the provided source numbers.',
    '- Prefer original sources (official sites, manufacturers, peer-reviewed papers) over aggregator blogs.',
    '- If search results are stale or undated for a time-sensitive question, say so and offer to search again with a tighter recency filter.',
    '- If results conflict, note the disagreement and favor the more authoritative or recent source.',
    '',
    'DO NOT:',
    '- Never invent, guess, or fabricate URLs, image filenames, or source citations. Only cite what appears in the actual search results.',
    '- Never include images using markdown syntax unless you have a real, verified HTTPS URL from the search results. Do not guess filenames like "product.png" or "image.jpg".',
    '- Never claim to have seen pages you did not actually fetch.',
    '',
...
export function buildOpenClawSystemPrompt(context: OpenClawPromptContext): string {
  ...
  const lines: string[] = [
    ...
    'If you do not have a real tool, browser, file, or system action available, say so plainly and give the best manual path instead of pretending the action happened.',
    'When cited web or knowledge-base context is provided, use it directly and do not ignore it.',
    'Do not use web research for facts about the user\'s current machine, local files, repository state, running processes, or installed software when shell or filesystem tools can answer directly.',
    'Keep the response presentation-ready. Use headings or lists only when they improve readability.',
    'Do not invent system access, local files, web results, or completed actions that you have not actually been given.',
    toolLabels.length > 0
  ...
  if (context.internetToolEnabled) {
    lines.push(
      'WEB RESEARCH CAPABILITY: You can request read-only public web research with citation-ready source context.',
      '',
      'SEARCH-FIRST RULE: For any question about current events, prices, products, software, people, statistics, news, health, legal matters, or anything that could have changed since your training cutoff, search BEFORE answering from memory. It is better to search and confirm than to answer incorrectly.',
      '',
      'WHEN TO SEARCH:',
      '- Any question about the current year, recent events, or things that change over time.',
      '- Product recommendations, comparisons, reviews, pricing, or availability.',
      '- Software versions, compatibility, bugs, or updates.',
      '- People, organizations, or policies that may have changed.',
      '- "How to" questions where procedures or tools may have changed.',
      '- Local, travel, weather, or location-specific queries.',
      '- The user explicitly asks for sources or verification.',
      '',
      'WHEN NOT TO SEARCH:',
      '- Stable knowledge: math, coding patterns, grammar, history, science fundamentals.',
      '- Questions about the user\'s own machine, files, or running processes (use shell/filesystem tools instead).',
      '- Creative writing, brainstorming, or opinion questions with no factual answer.',
      '',
      'QUERY RULES:',
      '- Write queries as concise search phrases (3-8 words), not questions.',
      '- Be specific: include years, model numbers, product names, locations, or domain terms.',
      '- Bad: "best mouse". Good: "best budget gaming mouse under $50 2025".',
      '- Bad: "python error". Good: "python TypeError NoneType has no attribute split".',
      '- Do not use search operators (site:, -, quotes) unless the user explicitly asks.',
      '- If a search returns irrelevant results (dictionary definitions, off-topic pages), refine with a narrower query rather than giving up.',
      '- For multi-faceted questions, break them into separate searches rather than one vague query.',
      '',
      'MULTI-STEP RESEARCH:',
      '- After each search result, evaluate whether you can answer the question or need more information.',
      '- If results are incomplete, conflicting, or stale, issue another search with a refined query.',
      '- For deep research tasks, plan 2-4 searches covering different angles before synthesizing.',
      '- You can also pass a specific URL in the query field to fetch and read a particular page.',
      '',
      'USING RESULTS:',
      '- Read source snippets carefully before answering. Do not skim.',
      '- Cite factual claims with inline markers like [^1] using the provided source numbers.',
      '- Prefer original sources (official sites, manufacturers, primary sources) over aggregator blogs.',
      '- If search results conflict, note the disagreement and favor the more authoritative or recent source.',
      '- If no relevant results are found, say so honestly. Do not fill gaps with speculation.',
      '',
      'DO NOT:',
      '- Never invent, guess, or fabricate URLs, image filenames, or source citations. Only cite what appears in actual search results.',
      '- Never include images using markdown syntax unless you have a real, verified HTTPS URL from the search results. Do not guess filenames like "product.png" or "response.txt".',
      '- Never claim to have seen pages you did not actually fetch.',
      '- Never repeat the same search query if the first search already returned clear results.',
      '',
      ...
      'The search backend uses multiple engines (Google, Brave, SearXNG, DuckDuckGo, Bing) with automatic fallback.',
      `Use this exact format:\n${OPENCLAW_WEB_TOOL_EXAMPLE}`,
    );
  }
