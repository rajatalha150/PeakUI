...
export function buildChatInternetToolPrompt(): string {
  return [
    ...
    'When writing search queries, be specific and use domain-appropriate terms. For example, write "best stocks to short 2025 market analysis" not just "best stock to short". Include years, specific names, and context keywords to get relevant results instead of dictionary definitions.',
    'The search backend uses multiple engines (Google, Brave, SearXNG, DuckDuckGo, Bing) and will find the best available results. Write your query as a clear search phrase, not a question.',
...
export function buildOpenClawSystemPrompt(context: OpenClawPromptContext): string {
  ...
  if (context.internetToolEnabled) {
    lines.push(
      'WEB RESEARCH CAPABILITY: You can request read-only public web research with citation-ready source context.',
      'Use this tool for current information, follow-up verification, narrower searches, deeper coverage, or when you need to fetch a specific public page.',
      'If web research for the current question was already attached in the latest system messages, use that first. Request another web search only when you need fresher evidence, a new angle, or a tighter query.',
      'The web tool can accept a normal search query or a public HTTP/HTTPS URL in the query field when you want a specific page fetched.',
      'Private, local, and non-public network targets are blocked.',
      'When writing search queries, be specific and use domain-appropriate terms. For example, write "best stocks to short 2025 market analysis" not just "best stock to short". Include years, specific names, and context keywords to get relevant results instead of dictionary definitions.',
      'The search backend uses multiple engines (Google, Brave, SearXNG, DuckDuckGo, Bing) and will find the best available results. Write your query as a clear search phrase, not a question.',
