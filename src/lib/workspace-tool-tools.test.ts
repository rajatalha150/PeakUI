...
describe('workspace-tool tool parsing', () => {
  ...
  it('strips both modern and legacy browser tool tags from content', () => {
    const modern = '<workspace_tool name="unified_browser">{"action":"search","query":"x"}</workspace_tool>'
  ...
  it('parses structured pdf_document requests without markdown content', () => {
    const input = [
      '<workspace_tool name="pdf_document">',
      JSON.stringify({
  ...
  it('parses structured workbook_document requests', () => {
    const input = [
      '<workspace_tool name="workbook_document">',
      JSON.stringify({
  ...
  it('parses structured word_document requests', () => {
    const input = [
      '<workspace_tool name="word_document">',
      JSON.stringify({
  ...
  it('accepts a title-only csv_document request (structure is recommended, not required)', () => {
    const input = [
      '<workspace_tool name="csv_document">',
      JSON.stringify({
        title: 'Customer List',
  ...
  it('accepts a title-only email_document request (structure is recommended, not required)', () => {
    const input = [
      '<workspace_tool name="email_document">',
      JSON.stringify({
        title: 'Follow-up',
  ...
  it('accepts a title-only markdown_document request (structure is recommended, not required)', () => {
    const input = [
      '<workspace_tool name="markdown_document">',
      JSON.stringify({
        title: 'Release Notes',
  ...
  it('drops malformed archive_document entries missing name or content', () => {
    const input = [
      '<workspace_tool name="archive_document">',
      JSON.stringify({
        title: 'Bundle',
        entries: [
  ...
  it('drops calendar_document events missing start', () => {
    const input = [
      '<workspace_tool name="calendar_document">',
      JSON.stringify({
        title: 'Schedule',
        events: [
  ...
  it('parses slides_document, archive_document, calendar_document, mermaid_document tool names', () => {
    ...
    for (const [name, payload] of Object.entries(payloads)) {
      const input = `<workspace_tool name="${name}">${payload}</workspace_tool>`
      const extracted = extractWorkspaceToolRequest(input)
  ...
  it('accepts filesystem payloads synthesized from prose narration (stat/read/list)', () => {
    for (const action of ['stat', 'read', 'list'] as const) {
      ...
      const input = `<workspace_tool name="filesystem">${payload}</workspace_tool>`
      const extracted = extractWorkspaceToolRequest(input)
  ...
  it('still parses a correctly formed modern <workspace_tool> after stripping junk wrappers', () => {
    const input = [
      'Some prose.',
      '<function_calls>',
...
describe('template placeholder guard', () => {
  it('rejects a copied <https URL> placeholder in unified_browser open (no broken dispatch)', () => {
    const input = '<workspace_tool name="unified_browser">{"action":"open","url":"<https URL>","description":"<what to inspect>"}</workspace_tool>'
    const extracted = extractWorkspaceToolRequest(input)
    expect(extracted.request).toBeUndefined()
  })

  it('rejects a copied <https URL> placeholder in the browser tool', () => {
    const input = '<workspace_tool name="browser">{"action":"open","url":"<https URL>"}</workspace_tool>'
    const extracted = extractWorkspaceToolRequest(input)
    expect(extracted.request).toBeUndefined()
  })

  it('rejects a copied <value> placeholder for web query', () => {
    const input = '<workspace_tool name="web">{"query":"<value>"}</workspace_tool>'
    const extracted = extractWorkspaceToolRequest(input)
    expect(extracted.request).toBeUndefined()
  })

  it('rejects a copied <command to run> placeholder for shell', () => {
    const input = '<workspace_tool name="shell">{"command":"<command to run>","description":"<what it does>"}</workspace_tool>'
    const extracted = extractWorkspaceToolRequest(input)
    expect(extracted.request).toBeUndefined()
  })

  it('rejects a copied <path> placeholder for filesystem', () => {
    const input = '<workspace_tool name="filesystem">{"action":"read","path":"<path>"}</workspace_tool>'
    const extracted = extractWorkspaceToolRequest(input)
    expect(extracted.request).toBeUndefined()
  })
  ...
  it('still accepts a real unified_browser open with a proper URL', () => {
    const input = '<workspace_tool name="unified_browser">{"action":"open","url":"https://stockanalysis.com/stocks/pltr/"}</workspace_tool>'
    const extracted = extractWorkspaceToolRequest(input)
    expect(extracted.request?.name).toBe('unified_browser')
...
describe('bare-domain URL promotion from prose', () => {
  it('promotes a bare domain in "open <domain>" narration (no scheme)', () => {
    const extracted = extractWorkspaceToolRequest('I\'ll open stockanalysis.com for the PLTR forecast.')
    expect(extracted.request?.name).toBe('unified_browser')
    if (extracted.request?.name !== 'unified_browser') throw new Error('expected ub')
  ...
  it('promotes a bare domain with a path', () => {
    const extracted = extractWorkspaceToolRequest('Now open marketbeat.com/stocks/NASDAQ/PLTR/forecast/')
    expect(extracted.request?.name).toBe('unified_browser')
    if (extracted.request?.name !== 'unified_browser') throw new Error('expected ub')
  ...
  it('still recovers a full https URL from prose', () => {
    const extracted = extractWorkspaceToolRequest('Let me visit https://example.org/page to check the listing.')
    expect(extracted.request?.name).toBe('unified_browser')
    if (extracted.request?.name !== 'unified_browser') throw new Error('expected ub')
  ...
  it('does not promote a non-domain token', () => {
    const extracted = extractWorkspaceToolRequest('Let me check the latest numbers now.')
    expect(extracted.request).toBeUndefined()
  })
})
...
describe('"Search query:" longhand recovery from prose', () => {
  it('recovers a unified_browser search from "Search query: X" narration', () => {
    const extracted = extractWorkspaceToolRequest(
  ...
  it('trims trailing punctuation from the query', () => {
    const extracted = extractWorkspaceToolRequest('Search query: PLTR price target consensus.')
    expect(extracted.request?.name).toBe('unified_browser')
  ...
  it('accepts an equals sign and quoted query', () => {
    const extracted = extractWorkspaceToolRequest('Search query="NVDA analyst ratings July 2026"')
    expect(extracted.request?.name).toBe('unified_browser')
  ...
  it('does not fire on ordinary prose that merely contains the word "search"', () => {
    const extracted = extractWorkspaceToolRequest('I will run a search for that next.')
    expect(extracted.request).toBeUndefined()
  })
})
...
describe('detectMalformedToolWrapper', () => {
  ...
  it('returns null for a correct workspace_tool block', () => {
    expect(detectMalformedToolWrapper('<workspace_tool name="web">{"query":"x"}</workspace_tool>')).toBeNull()
  })
