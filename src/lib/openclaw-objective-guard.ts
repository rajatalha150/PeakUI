 ...
 */
export function extractSearchSignal(request: OpenClawToolRequest): { signal: string; kind: string } | null {
  if (request.name === 'web') {
 ...
 *
 * Plain text only — never contains `<openclaw_tool>` or angle-bracketed
 ...
 *
 * Plain text only — no `<openclaw_tool>` and no angle-bracketed placeholder
