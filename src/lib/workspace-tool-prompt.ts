import type { WorkspaceToolProvider } from './settings';
import type { PromptTier } from './model-context';
import type { ShellExecutionTarget } from './shell-execution';
import { buildPersonaBrief, buildUserProfileBrief, type WorkspaceToolPersona, type WorkspaceToolUserProfile } from './workspace-tool-persona';
import { getWorkspaceToolWorkspaceContainerRoot, getWorkspaceToolWorkspaceHostRoot } from './workspace-tool-workspace';
import {
  WORKSPACE_TOOL_BROWSER_TOOL_EXAMPLE,
  WORKSPACE_TOOL_CODE_TOOL_EXAMPLE,
  WORKSPACE_TOOL_FILESYSTEM_TOOL_EXAMPLE,
  WORKSPACE_TOOL_FILESYSTEM_WRITE_TOOL_EXAMPLE,
  WORKSPACE_TOOL_SHELL_TOOL_EXAMPLE,
  WORKSPACE_TOOL_UWAF_BROWSER_TOOL_EXAMPLE,
  WORKSPACE_TOOL_WEB_TOOL_EXAMPLE,
} from './workspace-tool-tools';
import { buildCapabilityPromptLines, listActiveCapabilityLabels, listCapabilitySignatures, selectCapabilityIdsForQuery } from './workspace-tool-capabilities';
...
export interface WorkspaceToolPromptContext {
   ...
   * complete instructions and examples for the core tools, with document
   * capability examples query-gated. 'standard' is identical to 'full'.
   * 'compact' emits one-line availability notes for tools the latest query
   * does not obviously need. 'minimal' always emits one-line notes, drops the
   * verbose core-protocol rules, and omits document examples — for very small
   * local models whose context window cannot fit the full manifest next to the
   * conversation.
   */
...
export function buildWorkspaceToolSystemPrompt(context: WorkspaceToolPromptContext): string {
  ...
  const lines: string[] = [
    ...
    'TOOL CALL PROTOCOL (most important rule): When you intend to call a tool, the VERY FIRST thing in your reply must be the complete tool block — no preamble, no narration. The runtime only acts on a `<workspace_tool name="...">{json}</workspace_tool>` wrapper. Emit exactly one block per reply; request the next tool only after the previous result returns.',
    'Correct: <workspace_tool name="web">{"query":"palantir q2 2026 earnings"}</workspace_tool>',
    ...
    'When a tool is unavailable, say so and suggest a manual path. When cited web or knowledge-base context is provided, use it directly and do not ignore it.',
    workspace
      ? `Current selected workspace: ${workspace.name} at ${workspace.hostPath} (managed relative path: ${workspace.relativePath}). ALWAYS create files, folders, and archives inside this selected workspace. Never write project files or ZIP outputs to the parent workspace root or to unrelated paths such as ${getWorkspaceToolWorkspaceHostRoot()}/projects or ${getWorkspaceToolWorkspaceHostRoot()}/*.zip unless the user explicitly names that location. If the selected workspace is ${getWorkspaceToolWorkspaceHostRoot()}/users/<id>/workspaces/default, place deliverables under that path so they appear in the user's file tree and can be downloaded.`
      : context.workspaceHostRoot || context.workspaceHostRoot === ''
        ? `Account default workspace root: ${context.workspaceHostRoot || getWorkspaceToolWorkspaceHostRoot()}. Treat this as the project root when the user asks about "the workspace" or a named project without giving a full path.`
    ...
    ...(minimal ? [] : [
      ...
      '(7) NEVER end a message with bare action narration ("Let me check…", "Next step: render…") without the matching <workspace_tool> wrapper in the same message.',
  ...
  if (context.shellEnabled) {
    ...
    if (shouldEmitFullToolSection(tier, shellIntent)) {
      lines.push(
        ...
        shellTarget === 'host'
          ? `Host shell commands are constrained by approval rules, timeouts, output caps, and approved working-directory roots. The managed WorkSpaces workspace is available at ${getWorkspaceToolWorkspaceHostRoot()}.`
        ...
        shellTarget === 'host'
          ? `When you need the shared workspace, prefer ${getWorkspaceToolWorkspaceHostRoot()}.`
          : `The managed WorkSpaces workspace is available to shell at ${getWorkspaceToolWorkspaceHostRoot()} (host-style alias) and ${getWorkspaceToolWorkspaceContainerRoot()} (container path).`,
  ...
  if (filesystemWriteAvailable) {
    ...
    if (shouldEmitFullToolSection(tier, fsWriteIntent)) {
      lines.push(
        ...
        `CRITICAL: Always use an absolute host path under an approved writable root.${workspace ? ` The active selected workspace root is ${workspace.hostPath} (managed relative path: ${workspace.relativePath}); place ALL project files and folders under this per-user workspace, never at the shared parent root — e.g. ${workspace.hostPath}/<project>/file.ext` : ` The account default workspace root is ${context.workspaceHostRoot || getWorkspaceToolWorkspaceHostRoot()}; for projects inside the active workspace, prefix the path with that root (e.g. ${context.workspaceHostRoot || getWorkspaceToolWorkspaceHostRoot()}/projects/<project>/file.ext)`}.`,
  ...
  if (uwafBrowserAvailable && context.internetToolEnabled) {
    ...
    if (shouldEmitFullToolSection(tier, uwafIntent)) {
      ...
      lines.push(
        ...
        `Use this exact format:\n${WORKSPACE_TOOL_UWAF_BROWSER_TOOL_EXAMPLE}`,
        'Supported unified_browser actions: search, open, snapshot, click, type, press, wait_for_selector, scroll, back, forward, new_tab, list_tabs, switch_tab, close_tab, select, hover, extract, extract_table, research_batch, fill, submit, wait_for_user, reopen_recent. After opening a JS-heavy page or before interacting with controls, call snapshot; use returned targetRef values for click/type/select/hover/press instead of inventing CSS selectors. Re-snapshot after navigation or a page-changing action.',
        ...
        '  <workspace_tool name="unified_browser">{"action":"search","query":"transmission repair cheat sheet","browserMode":"direct"}</workspace_tool>',
