...
import { buildWorkspaceToolSystemPrompt } from './workspace-tool-prompt'
...
async function ensureAutomationSession(userId: string, requestedSessionId: string | null | undefined, title: string) {
  if (requestedSessionId) {
    const existing = await prisma.chatSession.findFirst({
      where: { id: requestedSessionId, userId, surface: 'workspace-tool' },
    })
...
async function executeAutomationRun(runId: string) {
  ...
  const resolvedModel = settings.workspaceToolAutomationExecutionModel.trim()
    || (settings.workspaceToolProvider === 'ollama' ? settings.workspaceToolModel.trim() : '')

  if (!settings.workspaceToolAutomationExecutionEnabled) {
  ...
  if (recentRunCount > settings.workspaceToolAutomationExecutionMaxRunsPerHour) {
    const error = `Skipped unattended run because the ${settings.workspaceToolAutomationExecutionMaxRunsPerHour}/hour execution budget was reached.`
  ...
  try {
    ...
    const workspaceContext = settings.workspaceToolAutomationExecutionAttachWorkspace && run.workspaceId
      ? await getWorkspaceToolWorkspaceContext(run.userId, run.workspaceId).catch(() => null)
      : null

    const [recentMemories, longTermMemory] = settings.workspaceToolAutomationExecutionAttachMemory
      ? await Promise.all([loadRecentMemory(2), loadLongTermMemory()])
      : [[], '']
    const memoryContext = settings.workspaceToolAutomationExecutionAttachMemory
    ...
    const systemPrompt = buildWorkspaceToolSystemPrompt({
      provider: 'ollama',
      model: resolvedModel,
