...
export async function createCalendarCanvasArtifact(input: {
  tx: Prisma.TransactionClient
  userId: string
  sessionId: string
  messageId?: string | null
  name: string
  calendarBytes: Buffer
  ...
  if (input.source && !sourceArtifactId) {
    const sourceMetadata = computeCanvasArtifactMetadata({
      ...
      bundleId,
      bundleName: input.bundleName ?? 'Generated Calendar',
      bundleRole: input.source.bundleRole ?? 'calendar-source',
    })
    ...
    const source = await input.tx.canvasArtifact.create({
      data: {
        ...
        previewSummary: sourceMetadata.previewSummary || 'Calendar source document',
  ...
  const metadata = computeCanvasArtifactMetadata({
    ...
    bundleId,
    bundleName: input.bundleName ?? 'Generated Calendar',
    bundleRole: input.bundleRole ?? 'generated-calendar',
  })
  ...
  const created = await input.tx.canvasArtifact.create({
    data: {
      ...
      kind: 'calendar',
      extension: 'ics',
      size: input.calendarBytes.byteLength,
      sessionId: input.sessionId,
      messageId: input.messageId ?? null,
      userId: input.userId,
      previewKind: metadata.previewKind,
      previewSummary: metadata.previewSummary || 'Generated calendar event',
