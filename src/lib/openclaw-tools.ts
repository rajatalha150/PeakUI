...
function isWordDocumentTemplate(value: unknown): value is WordDocumentTemplate {
  return value === 'report' || value === 'memo' || value === 'letter' || value === 'proposal' || value === 'contract' || value === 'resume' || value === 'checklist' || value === 'form'
}

const TOOL_BLOCK_PATTERN = /<openclaw_tool\s+name=["'](shell|filesystem|web|code|browser|unified_browser|tax_return|pdf_document|workbook_document|word_document)["']\s*>([\s\S]*?)<\/openclaw_tool>/i
...
export function stripAllToolTags(content: string): string {
  ...
  let cleaned = content.replace(/<openclaw_tool\s+name=["'](shell|filesystem|web|code|browser|unified_browser|tax_return|pdf_document|workbook_document|word_document)["']\s*>[\s\S]*?<\/openclaw_tool>/gi, '')
  cleaned = cleaned.replace(/<unified_browser>\s*[\s\S]*?<\/unified_browser>/gi, '')
  // Remove partial/incomplete tags (no closing tag)
  cleaned = cleaned.replace(/<openclaw_tool\s+name=["'](shell|filesystem|web|code|browser|unified_browser|tax_return|pdf_document|workbook_document|word_document)["']\s*>[\s\S]*/gi, '')
