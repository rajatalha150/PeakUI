...
} from './openclaw-objective-guard'
import type { OpenClawToolRequest } from './openclaw-tools'

function web(query: string): OpenClawToolRequest {
  return { name: 'web', request: { query } }
}

function ubSearch(query: string): OpenClawToolRequest {
  return { name: 'unified_browser', request: { action: 'search', query, browserMode: 'direct' } }
}

function ubOpen(url: string): OpenClawToolRequest {
...
describe('extractSearchSignal', () => {
  ...
  it('returns null for non-search tools', () => {
    const shell: OpenClawToolRequest = { name: 'shell', request: { command: 'ls' } }
