 ...
 * in prose but never emits the matching `<workspace_tool>` wrapper. The runtime
...
export function synthesizeToolCallFromNarration(
  ...
  const cleaned = content.replace(/<workspace_tool[\s\S]*?<\/workspace_tool>/gi, '').trim()
...
function synthesizeDocumentRegenerate(
  ...
  if (!WORKSPACE_TOOL_NAME_SET.has(lastSuccessful.name)) return null
 ...
 * WorkspaceToolRequest suitable for the existing dispatcher. Pulled out so
 * the synthesizer itself stays a pure function and is trivial to test.
 */
export function buildWorkspaceToolRequestFromNarration(
  recovery: NarrationRecovery,
): WorkspaceToolRequest | null {
  if (!WORKSPACE_TOOL_NAME_SET.has(recovery.toolName)) return null
  ...
  if (recovery.matchedPattern === 'document.regenerate') {
    ...
    if (resubmit && typeof resubmit === 'object') {
      return {
        name: recovery.toolName as WorkspaceToolRequest['name'],
