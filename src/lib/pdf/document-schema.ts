...
function normalizeSection(value: unknown): PdfDocumentSection | null {
  ...
  const section: PdfDocumentSection = {
    ...
    body: cleanString(record.body, 8000),
    ...
    tables: Array.isArray(record.tables) ? record.tables.map(normalizeTable).filter((table): table is PdfDocumentTable => Boolean(table)).slice(0, 8) : undefined,
...
export function normalizePdfDocumentInput(input: PdfDocumentInput): NormalizedPdfDocument {
  ...
  const content = typeof input.content === 'string' ? input.content.trim().slice(0, 60000) : ''
  ...
  const tables = Array.isArray(input.tables) ? input.tables.map(normalizeTable).filter((table): table is PdfDocumentTable => Boolean(table)).slice(0, 12) : []
