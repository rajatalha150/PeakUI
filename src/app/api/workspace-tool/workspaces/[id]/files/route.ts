...
async function resolveWorkspace(authUserId: string, workspaceId: string) {
  const record = await getWorkspaceToolWorkspaceById(authUserId, workspaceId)
...
async function readDirEntries(
  ...
  async function walk(absDir: string, relDir: string, depthRemaining: number): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (childRel === '.workspace-tool-workspace.json') continue
...
export async function GET(request: NextRequest, context: RouteContext) {
  const access = await requireCurrentAuthWithPermissions([...REQUIRED_PERMISSIONS], {
    ...
    actionRequired: 'Grant the workspace-tool.filesystem permission in Settings -> User Management, then ensure approved filesystem paths are configured in your own Settings.',
  })
