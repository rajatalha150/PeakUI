...
import { getUserSettings, normalizeWorkspaceToolProvider } from '@/lib/settings';
...
function normalizeProvider(value: unknown) {
  return normalizeWorkspaceToolProvider(value);
}
...
export async function POST(req: Request) {
  try {
    ...
    const provider = normalizeProvider(body.provider ?? settings.workspaceToolProvider);
    const baseUrl = normalizeProviderBaseUrl(
      body.baseUrl ?? body.base_url ?? (provider === 'openai-compatible' ? settings.workspaceToolBaseUrl : settings.ollamaHost),
      provider,
      provider === 'openai-compatible' ? settings.workspaceToolBaseUrl || DEFAULT_OPENAI_COMPATIBLE_BASE_URL : settings.ollamaHost,
