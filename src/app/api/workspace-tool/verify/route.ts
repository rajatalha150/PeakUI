...
import { getUserSettings, normalizeWorkspaceToolProvider, normalizeOllamaHost } from '@/lib/settings';
...
export async function POST(req: Request) {
  try {
    ...
    const provider = normalizeWorkspaceToolProvider(body.provider ?? settings.workspaceToolProvider);
    const baseUrl = provider === 'openai-compatible'
      ? normalizeProviderBaseUrl(
          body.baseUrl ?? body.base_url ?? settings.workspaceToolBaseUrl,
          provider,
          settings.workspaceToolBaseUrl || DEFAULT_OPENAI_COMPATIBLE_BASE_URL,
        ) || DEFAULT_OPENAI_COMPATIBLE_BASE_URL
