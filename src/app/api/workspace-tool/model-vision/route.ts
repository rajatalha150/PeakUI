...
import { getUserSettings, normalizeOllamaHost, normalizeWorkspaceToolProvider } from '@/lib/settings'
...
export async function POST(req: Request) {
  try {
    ...
    const provider = normalizeWorkspaceToolProvider(body.provider ?? settings.workspaceToolProvider) === 'openai-compatible'
      ? 'openai-compatible'
      : 'ollama'

    const rawBaseUrl = body.base_url ?? body.baseUrl
      ?? (provider === 'openai-compatible' ? settings.workspaceToolBaseUrl : settings.ollamaHost)
