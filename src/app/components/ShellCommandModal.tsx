...
interface ShellCommandModalProps {
  ...
  toolKind: 'shell' | 'filesystem' | 'code' | 'browser' | 'unified_browser' | 'tax_return' | 'pdf_document' | 'workbook_document' | 'word_document' | 'csv_document' | 'email_document'
  onApprove: () => void
  onReject: () => void
  isOpen: boolean
...
function renderToolIcon(toolKind: ShellCommandModalProps['toolKind']) {
  ...
  if (toolKind === 'tax_return' || toolKind === 'pdf_document' || toolKind === 'workbook_document' || toolKind === 'word_document' || toolKind === 'csv_document' || toolKind === 'email_document') return <FileText size={24} style={{ color: 'var(--accent-primary)' }} />
