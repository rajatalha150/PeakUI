...
type WorkspaceToolProvider = 'ollama' | 'openai-compatible';
...
interface WorkspaceToolWorkspaceRecord {
  id: string;
  slug: string;
  name: string;
  description: string;
...
interface WorkspaceToolModel {
  name: string;
  model: string;
}

interface WorkspaceToolMessage {
  id?: string;
...
type WorkspaceToolLatencyTimings = Record<string, string | number | boolean | null>;

interface WorkspaceToolImageAttachment {
  name: string;
  type: string;
  size: number;
  data: string;
...
interface WorkspaceToolFileAttachment {
  name: string;
  type: string;
  size?: number;
  extension?: string;
...
function getWorkspaceToolImageSrc(image: WorkspaceToolImageAttachment) {
  if (image.previewUrl) return image.previewUrl;
  const mimeType = image.type || 'image/jpeg';
  return `data:${mimeType};base64,${image.data}`;
}

function WorkspaceToolAttachedImagePreview({ image }: { image: WorkspaceToolImageAttachment }) {
...
interface WorkspaceToolSettings {
...
interface ParsedWorkspaceToolSettingsResponse {
...
function parseWorkspaceToolPermissions(value: unknown): EffectiveWorkspaceToolAccess['permissions'] {
  if (!Array.isArray(value)) return [];
  return value.filter(
...
function parseWorkspaceToolAccess(
  value: unknown,
...
/**
 * Parse the server's `workspaceToolFavoriteModels` payload into the client's
...
function parseWorkspaceToolSettingsResponse(data: Record<string, unknown>): ParsedWorkspaceToolSettingsResponse {
...
const WORKSPACE_TOOL_API_KEY_STORAGE = 'peakui-workspace-tool-api-key';
const WORKSPACE_TOOL_INTERNET_STORAGE = 'peakui-workspace-tool-internet-enabled';
const WORKSPACE_TOOL_RAG_STORAGE = 'peakui-workspace-tool-rag-enabled';
...
const WORKSPACE_TOOL_DRAFT_TASK_ID = '__draft__';
const WORKSPACE_TOOL_PERSONA_STORAGE = 'peakui-workspace-tool-persona';
const WORKSPACE_TOOL_USER_PROFILE_STORAGE = 'peakui-workspace-tool-user-profile';
const WORKSPACE_TOOL_CURRENT_SESSION_STORAGE = 'peakui-workspace-tool-current-session';
const WORKSPACE_TOOL_CURRENT_WORKSPACE_STORAGE = 'peakui-workspace-tool-current-workspace';
...
function getChatTitle(messages: WorkspaceToolMessage[]) {
...
function isVisibleMessage(message: WorkspaceToolMessage) {
...
function describeFilesystemRequest(action: WorkspaceToolFilesystemToolRequest['action'], requestedPath: string) {
...
function describeCodeExecutionRequest(request: WorkspaceToolCodeToolRequest) {
...
function describeBrowserRequest(request: WorkspaceToolBrowserToolRequest) {
...
function describeUwafBrowserRequest(request: WorkspaceToolUwafBrowserToolRequest) {
...
function describeTaxReturnRequest(request: WorkspaceToolTaxReturnToolRequest) {
...
function describePdfDocumentRequest(request: WorkspaceToolPdfDocumentToolRequest) {
  return request.description?.trim()
    ? `PDF generation: ${request.description.trim()}`
    : `Generating downloadable PDF: \`${request.filename || request.title}.pdf\``;
}

function describeWorkbookDocumentRequest(request: WorkspaceToolWorkbookDocumentToolRequest) {
  return request.description?.trim()
...
function describeWordDocumentRequest(request: WorkspaceToolWordDocumentToolRequest) {
  return request.description?.trim()
...
function describeCsvDocumentRequest(request: WorkspaceToolCsvDocumentToolRequest) {
  return request.description?.trim()
    ? `CSV export: ${request.description.trim()}`
    : `Generating downloadable CSV: \`${request.filename || request.title}.csv\``;
}

function describeEmailDocumentRequest(request: WorkspaceToolEmailDocumentToolRequest) {
  return request.description?.trim()
...
function describeFetchSummarizeRequest(request: WorkspaceToolFetchSummarizeToolRequest) {
  return request.description?.trim()
    ? `Fetch and summarize: ${request.description.trim()} (${request.url})`
    : `Fetch and summarize web page: ${request.url}`;
}

function describeMarkdownDocumentRequest(request: WorkspaceToolMarkdownDocumentToolRequest) {
  return request.description?.trim()
    ? `Markdown document: ${request.description.trim()}`
    : `Generating downloadable Markdown document: \`${request.filename || request.title}.md\``;
}

function describeSlidesDocumentRequest(request: WorkspaceToolSlidesDocumentToolRequest) {
  return request.description?.trim()
...
function describeArchiveDocumentRequest(request: WorkspaceToolArchiveDocumentToolRequest) {
  return request.description?.trim()
    ? `Archive: ${request.description.trim()}`
    : `Bundling ${request.entries.length} files into \`${request.filename || request.title}.zip\``;
}

function describeCalendarDocumentRequest(request: WorkspaceToolCalendarDocumentToolRequest) {
  return request.description?.trim()
    ? `Calendar event: ${request.description.trim()}`
    : `Creating calendar event(s): \`${request.filename || request.title}.ics\``;
}

function describeMermaidDocumentRequest(request: WorkspaceToolMermaidDocumentToolRequest) {
  return request.description?.trim()
...
function normalizeLatencyTimings(value: unknown): WorkspaceToolLatencyTimings | undefined {
...
function formatLatencySummary(timings?: WorkspaceToolLatencyTimings) {
  if (!timings) return '';
  const parts = [
...
function normalizeExtractedToolRequestName(value: unknown): WorkspaceToolMessage['toolRequest'] {
...
function normalizeWorkspaceToolMessage(value: unknown): WorkspaceToolMessage | null {
...
function normalizeWorkspaceToolSession(value: unknown): WorkspaceToolSession | null {
  if (!value || typeof value !== 'object') return null;
...
function sanitizeWorkspaceToolMessages(messages: unknown): WorkspaceToolMessage[] {
...
function stripAttachmentVisionData(messages: WorkspaceToolMessage[]): WorkspaceToolMessage[] {
...
function sanitizeWorkspaceToolSessions(sessions: unknown): WorkspaceToolSession[] {
...
function getLatestVisibleAssistantMessage(messages: WorkspaceToolMessage[]) {
...
function getWorkspaceToolRequestSignature(request: WorkspaceToolRequest) {
  if (request.name === 'web') {
...
// Extracts the short human-readable description and filename from a document
// tool request so the recovery/stall notice can tell the user which generator
// the model was trying to re-run. Returns null for non-document tools.
function describeDocumentToolRequest(request: WorkspaceToolRequest): { description?: string; filename?: string } | null {
...
function isDocumentToolName(name: WorkspaceToolRequest['name']): boolean {
...
// Human-friendly label for a tool name, used in the stall notice so the user
// sees "slide deck" instead of "slides_document".
function describeToolDisplayName(name: WorkspaceToolRequest['name']): string {
  switch (name) {
...
// fill in real content for its task. The shape is tailored to the named tool
// when known, with a generic fallback otherwise.
function buildRecoveryWrapperExample(toolName: WorkspaceToolRequest['name'] | undefined): string {
  switch (toolName) {
...
function pruneInterruptedMessages(messages: WorkspaceToolMessage[]) {
  const next = [...messages];
...
export default function WorkspaceToolWorkspace({
