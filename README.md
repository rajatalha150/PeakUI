...
18. Settings now includes Host Access presets plus a live status readout for mounted roots, approved read/write roots, and host-executor reachability. A filesystem `403` response includes a structured denial code and `actionRequired` guidance so you can tell whether the blocker is account permission, missing approved roots, a path outside Docker mounts, or a missing write approval token.
19. If you want WorkSpaces shell commands to run on the host instead of inside the container, start the optional host executor described in [WorkSpaces Host Executor](docs/openclaw-host-executor.md), set `OPENCLAW_HOST_EXECUTOR_TOKEN`, restart the app container, and switch `Shell Target` to `Host` in Settings.
20. If an WorkSpaces task needs a shared working directory, use `/tmp/peakui-openclaw-workspace` in host-style paths or `/mnt/openclaw/workspace` inside the container context.
...
22. If the app reports a model-memory error while the same model works in terminal, lower the context window in Settings. The default local path now lets Ollama choose its own context first, but explicit higher context requests are still capped to `4096` tokens by default and backed off further on memory pressure. Raise `PEAKUI_OLLAMA_CONTEXT_CAP` only on hardware that can actually carry larger KV caches.
23. If the browser reports a generation connection failure, the streamed error should now include the provider URL and socket cause. Test the same model locally with `ollama run <model> "hello"` when using Ollama, or confirm the remote compatible base URL and token when using a remote provider.
