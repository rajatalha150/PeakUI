...
- Uploaded PDFs retain their original bytes in `Document.originalContent` so downstream workflows such as tax-template filling can reconstruct fillable AcroForm PDFs after text indexing completes. Non-PDF uploads remain text/index oriented and do not store original binary bytes.
