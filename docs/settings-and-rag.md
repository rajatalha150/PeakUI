...
- Uploaded PDFs and workbook files (`.xlsx`, `.xlsm`, `.xls`) retain their original bytes in `Document.originalContent` so downstream workflows such as tax-template filling or workbook regeneration can reconstruct the original files after text indexing completes. Other uploads remain text/index oriented and do not store original binary bytes.
