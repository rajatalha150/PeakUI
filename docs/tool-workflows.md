...
PeakUI exposes several app-native tools so the AI can generate real deliverables instead of falling back to shell, filesystem, or the code sandbox. Each section below covers what the tool produces, when to use it, and the JSON/XML contract the model should emit inside an `<workspace_tool>` wrapper.
...
```xml
<workspace_tool name="slides_document">
{
  "title": "Q3 Review",
...
```xml
<workspace_tool name="mermaid_document">
{
...
```xml
<workspace_tool name="tax_return">
