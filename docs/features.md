...
- **User-configurable tool-step cap**: Settings exposes a "Tool-Step Cap Per Turn" slider (default 25) that controls how many real tool calls WorkSpaces can run in a single assistant turn before pausing for a "continue". Recovery nudges and duplicate warnings do not count toward the cap.
