...
- **Adaptive context window**: when `/api/show` reports a native window, PeakUI reads either Ollama metadata shape (`model_info`/`general` or `details.context_length`). With **Use Ollama/model default context** enabled, requests omit `num_ctx` and use that native window; memory-pressure accounting uses the same detected value. Manual context mode applies a model-aware cap, and the existing OOM backoff loop still steps down automatically if a raise is too aggressive for the GPU.
